...
线性区域为 $[0, 1.2]"A"$，拟合方程为 $y=b x+a.$
则其中 $y$ 为磁感应强度 $B$，$x$ 为电流强度．
...
1. 变小．
2.
*磁光隔离器*：在光纤通讯等场合中，反射光会严重干扰光源的正常输出，从而影响了整个系统的正常工作。磁光隔离器就是防止反向传输的干扰光对光源的影响，提高系统的工作稳定性。当光正向入射时，通过起偏器后成为线偏振光，再通过磁光介质与外磁场使光的偏振方向右旋45度，并恰好能通过与起偏器成45度放置的检偏器。而对于反向光，由检偏器射入的线偏振光经过放置介质时，偏转方向也右旋转45度，从而使反向光的偏振方向与起偏器方向成90度，无法通过起偏器，从而实现正向通过、反向隔离。
